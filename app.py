...
# Playwright -- untuk login bypass Cloudflare Turnstile (gratis, headless browser)
try:
...
def load_accounts():
    ...
    defaults = [
        ...
        {"email": "ceptampan58@gmail.com",       "password": "Encep1234"},
    ]
...
# ── 2captcha key untuk bypass Cloudflare Turnstile ──
# Daftar gratis di 2captcha.com → free trial 1000 solve
# Set env var TWOCAPTCHA_KEY di Railway dashboard
_TWOCAPTCHA_KEY = os.getenv("TWOCAPTCHA_KEY", "d074e113eaa89e185c8fbce7b641db12").strip()
# Legacy CapSolver (fallback kalau masih ada balance)
...
def _solve_turnstile_playwright(page_url, email, password):
    """
    Login iVAS via Playwright headless browser.
    Browser akan render halaman login, isi form, lalu Turnstile solve otomatis.
    Return dict: {ok, cookies, csrf, recv_csrf, live_html} atau {ok: False, error}
    """
    if not _PLAYWRIGHT_AVAILABLE:
        return {"ok": False, "error": "Playwright tidak tersedia — install dengan: pip install playwright && playwright install chromium"}

    logger.info(f"[PLAYWRIGHT] Login headless untuk {email}")
    try:
        with sync_playwright() as p:
            ...
            # Sembunyikan tanda automation dari Cloudflare
            ...
            logger.info(f"[PLAYWRIGHT] Form diisi untuk {email}")

            # Klik submit — Turnstile akan solve otomatis saat klik
            page.click('button[type="submit"]')

            # Tunggu redirect ke /portal atau kembali ke /login (maks 30 detik)
            try:
                page.wait_for_url("**/portal/**", timeout=30000)
                login_ok = True
            except PlaywrightTimeout:
                login_ok = "/portal" in page.url

            if not login_ok:
                err_text = ""
                try:
                    err_el = page.query_selector(".alert, .alert-danger, .invalid-feedback")
                    if err_el:
                        err_text = err_el.inner_text()
                except Exception:
                    pass
                browser.close()
                return {"ok": False, "error": f"Login gagal setelah Turnstile. URL={page.url}. Msg={err_text}"}

            logger.info(f"[PLAYWRIGHT] Login OK, URL={page.url}")

            # Ambil cookies dari browser untuk dipakai di requests session
            pw_cookies = ctx.cookies()

            # Ambil CSRF dari portal live
            live_html = ""
            csrf = ""
            try:
                ...
                meta = soup_live.find("meta", {"name": "csrf-token"})
                inp  = soup_live.find("input", {"name": "_token"})
                csrf = (meta["content"] if meta else (inp["value"] if inp else ""))
            except Exception as e:
                logger.warning(f"[PLAYWRIGHT] Gagal ambil CSRF live: {e}")

            # Ambil CSRF dari received
            recv_csrf = csrf
            try:
                page.goto(RECV_URL, wait_until="networkidle", timeout=20000)
                recv_html_str = page.content()
                soup_recv = BeautifulSoup(recv_html_str, "html.parser")
                recv_meta = soup_recv.find("meta", {"name": "csrf-token"})
                recv_inp  = soup_recv.find("input", {"name": "_token"})
                if recv_meta:
                    recv_csrf = recv_meta["content"]
                elif recv_inp:
                    recv_csrf = recv_inp["value"]
            except Exception as e:
                logger.warning(f"[PLAYWRIGHT] Gagal ambil recv_csrf: {e}")
        ...
        import traceback
        logger.error(f"[PLAYWRIGHT] Exception {email}: {e}\n{traceback.format_exc()}")
        return {"ok": False, "error": str(e)}


def _solve_turnstile(sitekey, page_url):
    """
    Solve Cloudflare Turnstile via 2captcha API.
    Set env var: TWOCAPTCHA_KEY=your_key
    Daftar gratis di 2captcha.com (1000 solve free trial)
    """
    key = os.getenv("TWOCAPTCHA_KEY", _TWOCAPTCHA_KEY).strip()
    if not key:
        logger.error("[TURNSTILE] TWOCAPTCHA_KEY tidak diset")
        return ""
    try:
        logger.info(f"[TURNSTILE] Solving via 2captcha... sitekey={sitekey[:30]}")

        # Step 1: submit task
        r1 = requests.post("https://2captcha.com/in.php", data={
            "key":        key,
            "method":     "turnstile",
            "sitekey":    sitekey,
            "pageurl":    page_url,
        ...
        d1 = r1.json()
        if d1.get("status") != 1:
            logger.error(f"[TURNSTILE] 2captcha submit gagal: {d1}")
            return ""
        task_id = d1.get("request", "")
        logger.info(f"[TURNSTILE] 2captcha taskId={task_id}, polling...")

        # Step 2: polling hasil
        time.sleep(5)  # tunggu minimal 5 detik sebelum poll pertama
        for _ in range(24):  # max 24 * 5 = 120 detik
            time.sleep(5)
            ...
            d2 = r2.json()
            if d2.get("status") == 1:
                token = d2.get("request", "")
                logger.info(f"[TURNSTILE] 2captcha SOLVED! token={token[:40]}...")
                return token
            if d2.get("request") == "CAPCHA_NOT_READY":
                continue
            logger.error(f"[TURNSTILE] 2captcha error: {d2.get('request')}")
            return ""
        logger.error("[TURNSTILE] 2captcha Timeout")
        return ""
    except Exception as e:
        logger.error(f"[TURNSTILE] Exception: {e}")
        return ""

def login_account(account):
    """
    Login satu akun via requests + 2captcha Turnstile solver.
    Return dict: {ok, scraper, csrf, recv_csrf, live_html, email}
    """
    email    = account["email"]
    password = account["password"]

    # ── Login via requests + 2captcha ──
    logger.info(f"[LOGIN] 2captcha solver untuk {email}")
    ...
    try:
        ...
        sitekey = ""
        ts_div  = soup.find("div", class_="cf-turnstile")
        if ts_div:
            sitekey = ts_div.get("data-sitekey", "")
        if not sitekey:
            sk_m = re.search(r'data-sitekey="([^"]{10,})"', page_html)
            if sk_m:
                sitekey = sk_m.group(1)

        turnstile_token = _solve_turnstile(sitekey, LOGIN_URL) if sitekey else ""

        login_data = {"email": email, "password": password, "_token": tok}
        if turnstile_token:
            login_data["cf-turnstile-response"] = turnstile_token
        ...
        if "/login" in getattr(resp, "url", ""):
            return {"ok": False, "error": "Login gagal (Turnstile required, solver gagal)", "email": email}
        ...
        try:
            ...
            if recv_meta:
                ...
                m2 = re.search(r"['\"]_token['\"]\s*[,:]?\s*['\"]([A-Za-z0-9_\-+/=]{20,})['\"]", recv_html)
                if m2:
                    recv_csrf = m2.group(1)
        except Exception as e2:
            logger.warning(f"[LOGIN] Gagal ambil recv_csrf {email}: {e2}")

        logger.info(f"[LOGIN] OK (CapSolver fallback) {email}")
        return {
            "ok":        True,
            "scraper":   scraper,
            "csrf":      csrf,
    ...
    try:
        ...
        if not sitekey:
            import re as _re2
            sk_m = _re2.search(r'data-sitekey="([^"]{10,})"', page_html)
            if sk_m:
                sitekey = sk_m.group(1)
        logger.info(f"[LOGIN] sitekey={'FOUND:'+sitekey[:25] if sitekey else 'NOT FOUND'} {email}")
        ...
        if turnstile_token:
            login_data["cf-turnstile-response"] = turnstile_token
            logger.info(f"[LOGIN] POST + Turnstile token {email}")
        else:
            logger.warning(f"[LOGIN] POST tanpa Turnstile token {email}")
        ...
        if "/login" in getattr(resp, "url", ""):
            ...
            if "turnstile" in resp_html.lower() and not turnstile_token:
                return {"ok": False, "error": "Turnstile required — solver gagal atau sitekey tidak ditemukan", "email": email}
            return {"ok": False, "error": "Email/password salah atau Turnstile error", "email": email}
...
def get_numbers(account, range_name, from_date, to_date, range_id=None):
    ...
    resp, _ = do_request(
        ...
        data={"start": to_ivas_start(from_date), "end": to_ivas_end(to_date), "range": range_name},
        headers=ajax_hdrs(),
    )
    ...
    resp2, _ = do_request(
        account, "POST",
        f"{BASE_URL}/portal/sms/received/getsms/number",
        data={"start": to_ivas_start(from_date), "end": to_ivas_end(to_date), "range": rid},
        headers=ajax_hdrs(),
    )
    if resp2 and resp2.status_code == 200:
        html2 = decode_response(resp2)
...
def _scrape_jwt_token(scraper, page_url: str):
...
def _build_ws_client(email: str, jwt_token: str) -> "_sio_module.Client | None":
    ...
    try:
        ...
        # ── Handler send_message_max_Limit — tangkap semua varian event ID ──
        @sio.on("send_message_max_Limit_231177")
        @sio.on("send_message_max_Limit_217253")
    ...
    try:
        ...
        def on_any_live(event, data):
            known = {"connect", "disconnect", "connect_error",
                     "send_message_live", "send_message_max_Limit_231177", "send_message_max_Limit_217253"}
            if livesms_event:
...
def get_ws_sms(email=None, limit: int = 100) -> list:
    """
...
def ws_oneshot_livesms(account: dict, timeout: int = 8) -> list:
    """
    One-shot WebSocket connect ke /livesms — cocok untuk Vercel serverless.

    Vercel tidak support persistent thread, jadi setiap request:
      1. Login (pakai cached session)
      2. Scrape JWT + user hash + dynamic event name dari HTML
      3. Connect Socket.IO ke /livesms
      4. Tunggu `timeout` detik untuk terima SMS
      5. Disconnect, return semua SMS yang masuk

    Return: list of SMS dicts yang diterima selama timeout
    """
    if not _SOCKETIO_AVAILABLE:
        logger.warning("[WS-1SHOT] python-socketio tidak tersedia")
        return []
    ...
    try:
        session = get_session(account)
        if not session or not session.get("ok"):
            logger.warning(f"[WS-1SHOT] {email} — login gagal")
            return []
        ...
        if not jwt_token:
            logger.warning(f"[WS-1SHOT] {email} — tidak ada JWT token")
            return []

        logger.info(
            f"[WS-1SHOT] {email} → Connect (event={'ada' if livesms_event else 'catch-all'})..."
        )
        ...
        def _parse(data):
            originator = str(data.get("originator", data.get("cli", ""))).replace("+", "")
            recipient  = str(data.get("recipient", data.get("number", "")))
            return {
                ...
                "recipient":    recipient,
                "message":      str(data.get("message", "")),
                "otp_message":  str(data.get("message", "")),
                "sid":          str(data.get("sid", "")),
        ...
        def _save(data):
            ...
            _ws_add_live(email, entry)  # juga simpan ke cache global
            logger.info(f"[WS-1SHOT] 📨 {email[:20]} from={entry['originator']} msg={entry['message'][:40]}")

        @sio.event
        def connect():
            logger.info(f"[WS-1SHOT] ✅ Connected {email}")

        @sio.event
        def disconnect():
            logger.info(f"[WS-1SHOT] Disconnected {email}")
            done_evt.set()

        # Register dynamic event (utama)
        if livesms_event:
            @sio.on(livesms_event)
            def on_dyn(data): _save(data)

        # Fallback static events
        @sio.on("send_message_live")
        def on_static(data): _save(data)

        @sio.on("send_message_max_Limit_231177")
        def on_limit(data): _save(data)
        ...
        def on_any(event, data):
            ...
            if livesms_event:
                known.add(livesms_event)
            if event not in known and isinstance(data, dict):
                if "message" in data or "originator" in data:
                    logger.info(f"[WS-1SHOT] catch-all event '{event[:40]}'")
                    _save(data)
        ...
        # Tunggu timeout detik untuk terima SMS
        ...
        logger.info(f"[WS-1SHOT] {email} selesai — {len(received)} SMS diterima")

    except Exception as e:
        logger.error(f"[WS-1SHOT] {email} exception: {e}")
...
def live_my_sms():
    ...
    # ws=1 → force oneshot WS, ws=0 → skip WS, default: auto
    ws_mode    = request.args.get("ws", "auto").strip()
    ws_timeout = int(request.args.get("ws_timeout", 6))

    def _apply_filters(items, source_label):
        clean = []
        for s in items:
            ...
            clean.append({
                "range":       s.get("range", ""),
                "number":      num,
                "originator":  s.get("originator", num),
    ...
    # ── Priority 1: WebSocket persistent cache (jalan di non-Vercel) ──
    if ws_mode != "0":
        ws_items = get_ws_live(acc_filter, limit=limit)
        if ws_items:
            clean = _apply_filters(ws_items, "websocket_livesms_cache")
            if clean:
                return jsonify({"status": "success", "source": "websocket_cache",
                                "accounts": len(targets), "total": len(clean), "sms": clean})

    # ── Priority 2: One-shot WebSocket (Vercel-friendly) ──
    # Connect langsung per-request, tunggu beberapa detik, ambil SMS
    if ws_mode != "0" and _SOCKETIO_AVAILABLE:
        oneshot_results = []
        seen_msgs = set()

        def _oneshot(acc):
            items = ws_oneshot_livesms(acc, timeout=ws_timeout)
            for item in items:
                key = (item.get("originator",""), item.get("message","")[:50])
                if key not in seen_msgs:
                    seen_msgs.add(key)
                    oneshot_results.append(item)

        with ThreadPoolExecutor(max_workers=min(len(targets), 3)) as ex:
            futs = [ex.submit(_oneshot, acc) for acc in targets]
            for f in as_completed(futs):
                try: f.result()
                except Exception as e: logger.warning(f"[MY-SMS] oneshot error: {e}")

        if oneshot_results:
            clean = _apply_filters(oneshot_results, "ws_oneshot")
            if clean:
                clean.sort(key=lambda x: x.get("received_at",""), reverse=True)
                return jsonify({"status": "success", "source": "ws_oneshot",
                                "accounts": len(targets), "total": len(clean), "sms": clean})
...
def debug_login_flow():
    ...
    # ── STEP 6: Full raw dump + SPA/Socket analysis ──
    if step_filter in ("all", "6"):
        out.append("STEP 6: RAW HTML DUMP + SPA/Socket.IO analysis")
        out.append(SEP2)
        try:
            sess6 = get_session(target_acc)
            if not sess6.get("ok"):
                out.append("ERROR: Session tidak tersedia")
            else:
                sc_s    = sess6["scraper"]
                today_s = datetime.now().strftime("%Y-%m-%d")
                start_s = f"{today_s} 00:00:00"
                end_s   = f"{today_s} 23:59:59"

                # 6A
                out.append("[ 6A ] GET /portal/sms/received")
                out.append(SEP2)
                try:
                    r6a = sc_s.get(RECV_URL, timeout=15)
                    h6a = decode_response(r6a)
                    out.append(f"Status: {r6a.status_code}  Size: {len(h6a)} chars")
                    out.append("[ Socket.IO io() ]")
                    for ln in h6a.split("\n"):
                        ls = ln.strip()
                        if "io(" in ls and "ivasms" in ls:
                            out.append(f"  {ls[:150]}")
                    out.append("[ socket.on ]")
                    for ln in h6a.split("\n"):
                        ls = ln.strip()
                        if "socket.on(" in ls:
                            out.append(f"  {ls[:120]}")
                    idx_gs = h6a.find("function GetSMS")
                    out.append("[ GetSMS() ]")
                    out.append(h6a[idx_gs:idx_gs+1200] if idx_gs >= 0 else "  (tidak ada)")
                    out.append(f"[ SPA  ] {{\'YES\' if \'SPA.navigate\' in h6a else \'NO\'}}")
                    out.append(f"[ WS   ] {{\'YES :2087\' if \'2087\' in h6a else \'NO\'}}")
                except Exception as e:
                    out.append(f"ERROR 6A: {e}")
                out.append(SEP)

                # 6B
                out.append("[ 6B ] POST /portal/sms/received/getsms")
                out.append(SEP2)
                h6b = ""
                ranges6b = []
                try:
                    csrf6b = _get_csrf_cached(sc_s, RECV_URL) or sess6.get("recv_csrf","")
                    r6b = sc_s.post(
                        f"{BASE_URL}/portal/sms/received/getsms",
                        data={"_token": csrf6b, "from": today_s, "to": today_s},
                        headers={"Accept":"text/html,*/*;q=0.01","X-Requested-With":"XMLHttpRequest",
                                 "Content-Type":"application/x-www-form-urlencoded; charset=UTF-8",
                                 "Referer":RECV_URL,"Origin":BASE_URL}, timeout=15)
                    h6b = decode_response(r6b)
                    out.append(f"Status: {r6b.status_code}  Size: {len(h6b)} chars")
                    out.append(f"CSRF  : {csrf6b[:60]}")
                    out.append("[ FULL RAW ]")
                    out.append(h6b)
                    for mm in re.finditer(r"toggleRange\s*\(\s*'([^']+)'\s*,\s*'([^']+)'\s*\)", h6b):
                        ranges6b.append({"name": mm.group(1), "id": mm.group(2)})
                    out.append(f"[ {len(ranges6b)} ranges ] {[r['name'] for r in ranges6b]}")
                except Exception as e:
                    out.append(f"ERROR 6B: {e}")
                out.append(SEP)

                # 6C
                out.append("[ 6C ] POST getsms/number (start/end HH:MM:SS CONFIRMED)")
                out.append(SEP2)
                h6c = ""
                nums6c = []
                try:
                    if not ranges6b:
                        out.append("  0 ranges — skip")
                    else:
                        rng6c  = ranges6b[0]
                        csrf6c = _get_csrf_cached(sc_s, RECV_URL) or sess6.get("recv_csrf","")
                        out.append(f"  Range: {rng6c['name']}  start={start_s}")
                        r6c = sc_s.post(
                            f"{BASE_URL}/portal/sms/received/getsms/number",
                            data={"_token": csrf6c, "start": start_s, "end": end_s,
                                  "range": rng6c["name"]},
                            headers={"Accept":"text/html,*/*;q=0.01","X-Requested-With":"XMLHttpRequest",
                                     "Content-Type":"application/x-www-form-urlencoded; charset=UTF-8",
                                     "Referer":RECV_URL,"Origin":BASE_URL}, timeout=15)
                        h6c = decode_response(r6c)
                        out.append(f"Status: {r6c.status_code}  Size: {len(h6c)} chars")
                        out.append("[ FULL RAW ]")
                        out.append(h6c)
                        for mm in re.finditer(r"toggleNum\w*\s*\(\s*'(\d{7,15})'\s*,\s*'([^']+)'\s*\)", h6c):
                            nums6c.append({"number": mm.group(1), "id": mm.group(2)})
                        out.append(f"[ {len(nums6c)} numbers ] {[n['number'] for n in nums6c]}")
                except Exception as e:
                    out.append(f"ERROR 6C: {e}")
                out.append(SEP)

                # 6D
                out.append("[ 6D ] POST getsms/number/sms")
                out.append(SEP2)
                try:
                    if not nums6c or not ranges6b:
                        out.append("  0 nomor — skip")
                    else:
                        num6d  = nums6c[0]
                        rng6d  = ranges6b[0]
                        csrf6d = _get_csrf_cached(sc_s, RECV_URL) or sess6.get("recv_csrf","")
                        out.append(f"  Number: {num6d['number']}  Range: {rng6d['name']}")
                        r6d = sc_s.post(
                            f"{BASE_URL}/portal/sms/received/getsms/number/sms",
                            data={"_token": csrf6d, "start": start_s, "end": end_s,
                                  "Number": num6d["number"], "Range": rng6d["name"]},
                            headers={"Accept":"text/html,*/*;q=0.01","X-Requested-With":"XMLHttpRequest",
                                     "Content-Type":"application/x-www-form-urlencoded; charset=UTF-8",
                                     "Referer":RECV_URL,"Origin":BASE_URL}, timeout=15)
                        h6d = decode_response(r6d)
                        out.append(f"Status: {r6d.status_code}  Size: {len(h6d)} chars")
                        out.append("[ FULL RAW ]")
                        out.append(h6d)
                        soup6d = BeautifulSoup(h6d, "html.parser")
                        msgs6d = [el.get_text(strip=True) for el in soup6d.select("div.msg-text,td.msg-text,p.msg-text")]
                        out.append(f"[ div.msg-text ] {msgs6d}")
                except Exception as e:
                    out.append(f"ERROR 6D: {e}")
                out.append(SEP)

        except Exception as e6:
            import traceback
            out.append(f"ERROR STEP 6: {e6}")
            out.append(traceback.format_exc())
        out.append(SEP)

    out.append("")
    out.append("Usage: /debug/login-flow?step=1|2|3|4|5|6|all&account=email@gmail.com")
    return Response("\n".join(out), mimetype="text/plain; charset=utf-8")
...
def debug_raw_v2():
    ...
    try:
        ...
        if method == "GET":
            r = sc_r.get(target_url, timeout=15)
        else:
...
def debug_master():
    ...
    out.append("[DEBUG MASTER] KY-SHIRO iVAS Full Diagnostic")
    out.append(f"Account  : {target_acc['email']}")
    out.append(f"Date     : {date_str}")
    out.append(f"Sections : {sections}")
    out.append(f"Dry run  : {dry_run}")
    out.append(f"All accs : {[a['email'] for a in ACCOUNTS]}")
    out.append(SEP)

    # ────────────────────────────────────────────────────────────────
    # SESSION — Login + ambil CSRF + cookies
    # ────────────────────────────────────────────────────────────────
    h("SESSION — Login + CSRF + Cookies")
    try:
        sess = get_session(target_acc)
        if not sess.get("ok"):
            out.append("ERROR: Login gagal — semua section di bawah akan skip")
            return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

        sc = sess["scraper"]
        out.append(f"Login    : OK")
        out.append(f"CSRF     : {(sess.get('csrf') or '')[:60]}")
        out.append(f"recv_csrf: {(sess.get('recv_csrf') or '')[:60]}")
        out.append("")
        out.append("[ Cookies ]")
        for k, v in dict(sc.cookies).items():
            out.append(f"  {k} = {str(v)[:80]}")
    except Exception as e:
        out.append(f"ERROR SESSION: {e}")
        return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

    # Helper headers
    def _ajax_hdrs(referer=RECV_URL):
        return {
            "Accept":           "text/html, */*; q=0.01",
            "X-Requested-With": "XMLHttpRequest",
            "Content-Type":     "application/x-www-form-urlencoded; charset=UTF-8",
    ...
    def _json_hdrs(referer=BASE_URL):
        return {
            "Accept":           "application/json, text/javascript, */*; q=0.01",
            "X-Requested-With": "XMLHttpRequest",
            "Content-Type":     "application/x-www-form-urlencoded; charset=UTF-8",
    ...
    csrf = lambda: _get_csrf_cached(sc, RECV_URL) or sess.get("recv_csrf","")

    start_dt = f"{date_str} 00:00:00"
    end_dt   = f"{date_str} 23:59:59"

    # ────────────────────────────────────────────────────────────────
    # SECTION 1: SMS RECEIVED — chain ranges → numbers → sms
    # ────────────────────────────────────────────────────────────────
    if want("received"):
        h("SMS RECEIVED — /portal/sms/received chain")

        # 1A: Shell page CSRF
        sub("1A: GET /portal/sms/received — scrape CSRF")
        try:
            r = sc.get(RECV_URL, timeout=15)
            h_html = decode_response(r)
            out.append(f"Status : {r.status_code}  Size: {len(h_html)} chars")
            fresh_csrf = csrf()
            out.append(f"CSRF   : {fresh_csrf[:60] if fresh_csrf else 'MISSING!'}")
        except Exception as e:
            out.append(f"ERROR 1A: {e}")

        # 1B: POST getsms → ranges
        sub("1B: POST /getsms — list ranges (from/to = date, no time)")
        h1b = ""
        ranges = []
        try:
            r = sc.post(f"{BASE_URL}/portal/sms/received/getsms",
                        data={"_token": csrf(), "from": date_str, "to": date_str},
                        headers=_ajax_hdrs(), timeout=15)
            h1b = decode_response(r)
            out.append(f"Status : {r.status_code}  Size: {len(h1b)} chars")
            out.append("")
            out.append("[ FULL RAW ]")
            out.append(h1b[:3000])
            if len(h1b) > 3000:
                out.append(f"... [truncated, total {len(h1b)} chars]")
            out.append("")
            # Parse ranges
            for m in re.finditer(r"toggleRange\s*\(\s*'([^']+)'\s*,\s*'([^']+)'\s*\)", h1b):
                ranges.append({"name": m.group(1), "id": m.group(2)})
            out.append(f"[ {len(ranges)} ranges parsed ]")
            for rng in ranges:
                out.append(f"  name={rng['name']}  id={rng['id']}")
        except Exception as e:
            out.append(f"ERROR 1B: {e}")

        # 1C: POST getsms/number → phone numbers (pakai DATETIME!)
        sub("1C: POST /getsms/number — list numbers per range (start/end dengan HH:MM:SS)")
        all_tasks = []
        for rng in ranges[:3]:  # max 3 range
            out.append(f"Range: {rng['name']}")
            try:
                r = sc.post(f"{BASE_URL}/portal/sms/received/getsms/number",
                            data={"_token": csrf(), "start": start_dt, "end": end_dt,
                                  "range": rng["name"]},
                            headers=_ajax_hdrs(), timeout=15)
                h1c = decode_response(r)
                out.append(f"  Status: {r.status_code}  Size: {len(h1c)} chars")
                out.append("  [ RAW (600 chars) ]")
                out.append("  " + h1c[:600].replace("\n","\n  "))
                nums = []
                for m in re.finditer(r"toggleNum\w*\s*\(\s*'(\d{7,15})'\s*,\s*'([^']+)'\s*\)", h1c):
                    nums.append({"number": m.group(1), "id": m.group(2)})
                out.append(f"  [ {len(nums)} numbers ] {[n['number'] for n in nums[:5]]}")
                for n in nums:
                    all_tasks.append({"number": n["number"], "range": rng["name"]})
            except Exception as e:
                out.append(f"  ERROR: {e}")

        # 1D: POST getsms/number/sms → actual messages
        sub("1D: POST /getsms/number/sms — actual SMS messages")
        if not all_tasks:
            out.append("  0 nomor → skip")
        else:
            for task in all_tasks[:3]:  # max 3 nomor
                out.append(f"Number: {task['number']}  Range: {task['range']}")
                try:
                    r = sc.post(f"{BASE_URL}/portal/sms/received/getsms/number/sms",
                                data={"_token": csrf(), "start": start_dt, "end": end_dt,
                                      "Number": task["number"], "Range": task["range"]},
                                headers=_ajax_hdrs(), timeout=15)
                    h1d = decode_response(r)
                    out.append(f"  Status: {r.status_code}  Size: {len(h1d)} chars")
                    out.append("  [ RAW (600 chars) ]")
                    out.append("  " + h1d[:600].replace("\n","\n  "))
                    soup = BeautifulSoup(h1d, "html.parser")
                    msgs = [el.get_text(strip=True) for el in soup.select("div.msg-text, td.msg-text, p.msg-text")]
                    out.append(f"  [ div.msg-text ] {msgs[:3]}")
                except Exception as e:
                    out.append(f"  ERROR: {e}")

    # ────────────────────────────────────────────────────────────────
    # SECTION 2: SMS LIVE — scrape + WebSocket status
    # ────────────────────────────────────────────────────────────────
    if want("live"):
        h("SMS LIVE — /portal/live/my_sms scrape + Socket.IO")

        sub("2A: GET /portal/live/my_sms — scrape JWT + user hash + event name")
        try:
            r = sc.get(LIVE_URL, timeout=15)
            h2a = decode_response(r)
            out.append(f"Status : {r.status_code}  Size: {len(h2a)} chars")
            out.append("")
            out.append("[ Socket.IO io() calls ]")
            for ln in h2a.split("\n"):
                ls = ln.strip()
                if "io(" in ls and ("ivasms" in ls.lower() or "socket" in ls.lower()):
                    out.append(f"  {ls[:160]}")
            out.append("")
            out.append("[ socket.on() events ]")
            for ln in h2a.split("\n"):
                ls = ln.strip()
                if "socket.on(" in ls or ".on(" in ls:
                    out.append(f"  {ls[:120]}")
            out.append("")
            # JWT / user hash
            jwt_tok, user_hash, live_event = _scrape_jwt_and_user(sc, LIVE_URL)
            out.append(f"[ JWT token  ] {(jwt_tok or 'NOT FOUND')[:80]}")
            out.append(f"[ user hash  ] {(user_hash or 'NOT FOUND')[:80]}")
            out.append(f"[ live event ] {(live_event or 'NOT FOUND')[:80]}")
            out.append("")
            out.append("[ Pusher/socket keys from JS ]")
            pat_key   = r"key:\s*['\"]([^'\"]{10,})['\"]"
            pat_token = r"token['\"]?\s*[:=]\s*['\"]([^'\"]{10,})['\"]"
            pat_user  = r"user['\"]?\s*[:=]\s*['\"]([a-f0-9]{20,})['\"]"
            for pat in [pat_key, pat_token, pat_user]:
                for m in re.finditer(pat, h2a):
                    out.append(f"  {m.group(0)[:100]}")
        except Exception as e:
            import traceback
            out.append(f"ERROR 2A: {e}")
            out.append(traceback.format_exc()[:500])

        sub("2B: WebSocket /ws/status — status koneksi")
        try:
            statuses = get_ws_status_all()
            for email, st in statuses.items():
                out.append(f"  {email[:30]:30s} connected={st.get('connected')}  "
                           f"live={st.get('live_connected')}  "
                           f"sms={st.get('sms_received',0)}  "
                           f"live_sms={st.get('live_sms_received',0)}  "
                           f"err={st.get('error','')[:40]}")
        except Exception as e:
            out.append(f"ERROR 2B: {e}")

        sub("2C: WS SMS Cache — sample 5 SMS dari cache")
        try:
            test_sms = get_ws_sms(limit=5)
            live_sms = get_ws_live(limit=5)
            out.append(f"Test SMS cache  : {len(get_ws_sms())} total")
            out.append(f"Live SMS cache  : {len(get_ws_live())} total")
            out.append("")
            out.append("[ Sample Test SMS (WS cache) ]")
            for s in test_sms[:3]:
                out.append(f"  from={s.get('originator','')} to={s.get('recipient','')} "
                           f"msg={s.get('message','')[:50]} src={s.get('source','')}")
            out.append("[ Sample Live SMS (WS cache) ]")
            for s in live_sms[:3]:
                out.append(f"  from={s.get('originator','')} to={s.get('recipient','')} "
                           f"msg={s.get('message','')[:50]} src={s.get('source','')}")
        except Exception as e:
            out.append(f"ERROR 2C: {e}")

        sub("2D: GET /portal/live/my_sms — scrape polling HTML (fallback)")
        try:
            r = sc.get(LIVE_URL, timeout=15)
            h2d = decode_response(r)
            soup2d = BeautifulSoup(h2d, "html.parser")
            # Cari tabel / row SMS
            rows2d = soup2d.select("tr.sms-row, div.sms-item, tbody tr")[:5]
            out.append(f"Status  : {r.status_code}  Size: {len(h2d)} chars")
            out.append(f"SMS rows: {len(rows2d)}")
            for row in rows2d[:3]:
                out.append(f"  {row.get_text(separator=' | ', strip=True)[:120]}")
        except Exception as e:
            out.append(f"ERROR 2D: {e}")

    # ────────────────────────────────────────────────────────────────
    # SECTION 3: ADD NUMBER — probe Test Numbers + dry run POST add
    # ────────────────────────────────────────────────────────────────
    if want("addnumber"):
        h("ADD NUMBER — /portal/numbers/test + add endpoint")

        sub("3A: GET /portal/numbers/test — raw HTML")
        test_rows = []
        sample_range = ""
        sample_tid   = ""
        try:
            r = sc.get(f"{BASE_URL}/portal/numbers/test", timeout=15)
            h3a = decode_response(r)
            out.append(f"Status : {r.status_code}  Size: {len(h3a)} chars")
            out.append("")
            out.append("[ JS datatables/ajax URL ]")
            for ln in h3a.split("\n"):
                ls = ln.strip()
                if "ajax" in ls.lower() and ("number" in ls.lower() or "datatables" in ls.lower()):
                    out.append(f"  {ls[:150]}")
            out.append("")
            out.append("[ RAW (800 chars) ]")
            out.append(h3a[:800])
        except Exception as e:
            out.append(f"ERROR 3A: {e}")

        sub("3B: POST /portal/numbers/test/datatables — fetch 5 nomor")
        try:
            rows3b, total3b = _fetch_datatables(
                target_acc, f"{BASE_URL}/portal/numbers/test", search="", length=5)
            out.append(f"Total iVAS: {total3b}  Fetched: {len(rows3b)}")
            out.append("")
            out.append("[ RAW row[0] keys ]")
            if rows3b:
                out.append(f"  Keys: {list(rows3b[0].keys())}")
                out.append(f"  Row0: {str(rows3b[0])[:300]}")
                for row in rows3b[:3]:
                    tid  = str(row.get("id","") or row.get("DT_RowId","") or _get_number_id(row))
                    num  = re.sub(r"<[^>]+>","",str(row.get("test_number",""))).strip()
                    rng  = re.sub(r"<[^>]+>","",str(row.get("range",""))).strip()
                    out.append(f"  termination_id={tid}  number={num}  range={rng}")
                    if not sample_tid and tid:
                        sample_tid   = tid
                        sample_range = rng
        except Exception as e:
            out.append(f"ERROR 3B: {e}")

        sub(f"3C: POST /portal/numbers/termination/number/add — {'DRY RUN' if dry_run else 'LIVE'}")
        out.append(f"  termination_id = {sample_tid or 'N/A'}")
        if not sample_tid:
            out.append("  SKIP — tidak ada termination_id dari 3B")
        elif dry_run:
            out.append("  DRY RUN → tidak di-POST. Jalankan dengan dry_run=0 untuk eksekusi.")
            out.append(f"  Payload yg akan dikirim: {{id: {sample_tid}}}")
            out.append(f"  URL: {BASE_URL}/portal/numbers/termination/number/add")
        else:
            try:
                r = sc.post(f"{BASE_URL}/portal/numbers/termination/number/add",
                            data={"id": sample_tid},
                            headers=_json_hdrs(f"{BASE_URL}/portal/numbers/test"),
                            timeout=15)
                h3c = decode_response(r)
                out.append(f"  Status   : {r.status_code}")
                out.append(f"  Response : {h3c[:400]}")
                try:
                    out.append(f"  JSON     : {r.json()}")
                except Exception:
                    pass
            except Exception as e:
                out.append(f"  ERROR: {e}")
    ...
    # ────────────────────────────────────────────────────────────────
    # SECTION 4: DELETE NUMBER — probe My Numbers + dry run POST delete
    # ────────────────────────────────────────────────────────────────
    if want("deletenumber"):
        h("DELETE NUMBER — /portal/numbers + return endpoint")

        sub("4A: GET /portal/numbers — raw HTML")
        try:
            r = sc.get(f"{BASE_URL}/portal/numbers", timeout=15)
            h4a = decode_response(r)
            out.append(f"Status : {r.status_code}  Size: {len(h4a)} chars")
            out.append("")
            out.append("[ JS ajax/datatables ]")
            for ln in h4a.split("\n"):
                ls = ln.strip()
                if ("ajax" in ls.lower() or "return" in ls.lower()) and "number" in ls.lower():
                    out.append(f"  {ls[:150]}")
            out.append("")
            out.append("[ RAW (500 chars) ]")
            out.append(h4a[:500])
        except Exception as e:
            out.append(f"ERROR 4A: {e}")

        sub("4B: POST My Numbers datatables — fetch 5 nomor")
        my_rows  = []
        sample_nid = ""
        try:
            my_rows, total4b = _fetch_my_numbers(target_acc, search="", length=5)
            out.append(f"Total iVAS: {total4b}  Fetched: {len(my_rows)}")
            if my_rows:
                out.append(f"[ RAW row[0] keys ]")
                out.append(f"  Keys: {list(my_rows[0].keys())}")
                out.append(f"  Row0: {str(my_rows[0])[:300]}")
                for row in my_rows[:3]:
                    nid  = _get_number_id(row)
                    num  = re.sub(r"<[^>]+>","",str(row.get("Number",row.get("number","")))).strip()
                    rng  = re.sub(r"<[^>]+>","",str(row.get("range",""))).strip()
                    out.append(f"  number_id={nid}  number={num}  range={rng}")
                    if not sample_nid and nid:
                        sample_nid = nid
        except Exception as e:
            out.append(f"ERROR 4B: {e}")

        sub(f"4C: POST /portal/numbers/return/number — {'DRY RUN' if dry_run else 'LIVE'}")
        out.append(f"  number_id = {sample_nid or 'N/A'}")
        if not sample_nid:
            out.append("  SKIP — tidak ada number_id dari 4B")
        elif dry_run:
            out.append("  DRY RUN → tidak di-POST. Jalankan dengan dry_run=0 untuk eksekusi.")
            out.append(f"  Payload yg akan dikirim: {{NumberID: {sample_nid}}}")
            out.append(f"  URL: {BASE_URL}/portal/numbers/return/number")
        else:
            try:
                r = sc.post(f"{BASE_URL}/portal/numbers/return/number",
                            data={"NumberID": sample_nid},
                            headers=_json_hdrs(f"{BASE_URL}/portal/numbers"),
                            timeout=15)
                h4c = decode_response(r)
                out.append(f"  Status   : {r.status_code}")
                out.append(f"  Response : {h4c[:400]}")
                try:
                    out.append(f"  JSON     : {r.json()}")
                except Exception:
                    pass
    ...
    # ────────────────────────────────────────────────────────────────
    # SECTION 5: WEBSOCKET — JWT scrape + connect test + event check
    # ────────────────────────────────────────────────────────────────
    if want("websocket"):
        h("WEBSOCKET — Socket.IO connect test + JWT + event names")

        sub("5A: Scrape JWT + user hash dari /portal/live/my_sms")
        jwt_tok = user_hash = live_event = None
        try:
            jwt_tok, user_hash, live_event = _scrape_jwt_and_user(sc, LIVE_URL)
            out.append(f"JWT token   : {(jwt_tok   or 'NOT FOUND')[:80]}")
            out.append(f"user hash   : {(user_hash or 'NOT FOUND')[:80]}")
            ...
            out.append(f"ERROR 5A: {e}")

        sub("5B: WS global status semua akun")
        try:
            statuses = get_ws_status_all()
            out.append(f"Total akun di WS : {len(statuses)}")
            for email, st in statuses.items():
                out.append(f"  {email[:35]:35s}  "
                           f"test_conn={st.get('connected',False)}  "
                           f"live_conn={st.get('live_connected',False)}  "
                           f"test_sms={st.get('sms_received',0)}  "
                           f"live_sms={st.get('live_sms_received',0)}  "
                           f"reconnects={st.get('reconnects',0)}  "
                           f"err={str(st.get('error',''))[:40]}")
        except Exception as e:
            out.append(f"ERROR 5B: {e}")

        sub("5C: WS connect URLs yang dipakai")
        out.append(f"  Test SMS WS  : wss://ivasms.com:2087  (namespace /)")
        out.append(f"  Live SMS WS  : wss://ivasms.com:2087  (namespace /livesms)")
        out.append(f"  Auth params  : token=JWT  user=userHash")
        out.append(f"  Events test  : send_message_test, send_message_live, send_message_max_Limit_231177")
        out.append(f"  Events live  : {{live_event}}, send_message_live, send_message_max_Limit_231177")
        out.append("")
        out.append(f"  live_event (scraped) : {live_event or 'MISSING — dynamic event tidak ketemu!'}")
        out.append("")
        if not live_event:
            out.append("  ⚠ WARNING: live_event kosong → SMS My SMS tidak akan masuk ke cache!")
            out.append("  → _scrape_jwt_and_user() perlu difix untuk tangkap event name dari JS")
        if not jwt_tok:
            out.append("  ⚠ WARNING: JWT token kosong → WS connect akan gagal auth!")

        sub("5D: Scrape /portal/live/my_sms raw JS — cari event name manual")
        try:
            r = sc.get(LIVE_URL, timeout=15)
            h5d = decode_response(r)
            out.append(f"Status : {r.status_code}  Size: {len(h5d)} chars")
            out.append("")
            out.append("[ Semua .on( / socket.on( calls ]")
            for ln in h5d.split("\n"):
                ls = ln.strip()
                if ".on(" in ls and "function" in ls:
                    out.append(f"  {ls[:140]}")
            out.append("")
            out.append("[ token / user assignments ]")
            p5_token  = r"""token\s*[=:]\s*['"]([^'",;]{10,})['"]"""
            p5_user   = r"""user\s*[=:]\s*['"]([a-f0-9$./+]{10,})['"]"""
            p5_echo   = r"""laravel_echo_.*?['"]([^'",;]{10,})['"]"""
            for pat in [p5_token, p5_user, p5_echo]:
                for m in re.finditer(pat, h5d):
                    out.append(f"  {m.group(0)[:120]}")
        except Exception as e:
            out.append(f"ERROR 5D: {e}")

    # ────────────────────────────────────────────────────────────────
    # RINGKASAN
    # ────────────────────────────────────────────────────────────────
    h("RINGKASAN & CARA PAKAI")
    out.append("Endpoints production:")
    out.append("  /sms?mode=received&date=DD/MM/YYYY   → SMS received semua akun")
    out.append("  /sms?mode=live                        → SMS live dari WS cache")
    out.append("  /ws/sms                               → WS test SMS cache")
    out.append("  /ws/status                            → Status koneksi WebSocket")
    out.append("  /ws/reconnect                         → Force reconnect WS")
    out.append("  /numbers/test-list                    → List Test Numbers")
    out.append("  /numbers/my-list                      → List My Numbers")
    out.append("  /numbers/add?range_name=TOGO+650      → Add nomor (dry_run=1 default)")
    out.append("  /numbers/delete?number_id=XXX         → Delete nomor")
    out.append("")
    out.append("Debug endpoints:")
    out.append("  /debug/master?sections=received        → debug SMS received saja")
    out.append("  /debug/master?sections=live            → debug SMS live saja")
    out.append("  /debug/master?sections=addnumber       → debug add number saja")
    out.append("  /debug/master?sections=deletenumber    → debug delete number saja")
    out.append("  /debug/master?sections=websocket       → debug websocket saja")
    out.append("  /debug/master?sections=all&dry_run=0   → semua + eksekusi add/delete!")
    out.append("")
    out.append("  /debug/getsms-raw?from=YYYY-MM-DD      → full raw chain received SMS")
    out.append("  /debug/raw?url=URL&method=POST         → hit URL iVAS apapun")
    out.append("  /debug/login-flow?step=6               → full login + STEP 6")
...
@app.route("/debug/capsolver")
def debug_capsolver():
    """Cek status CapSolver key: balance, validity, dan test solve Turnstile iVAS."""
    SEP = "=" * 60
    out = []
    out.append("[DEBUG 2CAPTCHA]")
    out.append(f"Key (partial) : {_CAPSOLVER_KEY[:25]}...")
    out.append(SEP)
    try:
        # 1. Cek balance
        out.append("--- 1. GET BALANCE ---")
        rb = requests.post("https://2captcha.com/res.php", json={
            "clientKey": _CAPSOLVER_KEY
        }, timeout=10)
        db = rb.json()
        out.append(f"HTTP status   : {rb.status_code}")
        out.append(f"Balance       : {db.get('balance', 'N/A')}")
        out.append(f"ErrorCode     : {db.get('errorCode', 'none')}")
        out.append(f"ErrorDesc     : {db.get('errorDescription', 'none')}")
        out.append(f"Full resp     : {db}")
        out.append("")

        if db.get("errorCode"):
            out.append("⚠ KEY BERMASALAH! Kemungkinan: key salah, expired, atau habis balance.")
            out.append("→ Solusi: ganti CAPSOLVER_KEY di Vercel env vars")
            return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

        bal = float(db.get("balance", 0))
        if bal <= 0:
            out.append("⚠ BALANCE = 0! Perlu top up di https://dashboard.capsolver.com")
            return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

        out.append(f"✅ Key valid, balance = {bal}")
        out.append("")

        # 2. Ambil sitekey iVAS
        out.append("--- 2. SCRAPE SITEKEY IVAS ---")
        sc = build_scraper()
        r1 = sc.get(LOGIN_URL, timeout=20)
        h1 = decode_response(r1)
        soup1 = BeautifulSoup(h1, "html.parser")
        ts_div  = soup1.find("div", class_="cf-turnstile")
        sitekey = ts_div.get("data-sitekey", "") if ts_div else ""
        if not sitekey:
            sk_m = re.search(r'data-sitekey="([^"]{10,})"', h1)
            sitekey = sk_m.group(1) if sk_m else ""
        out.append(f"Sitekey : {sitekey or 'NOT FOUND'}")
        out.append("")

        if not sitekey:
            out.append("⚠ Sitekey tidak ditemukan di halaman login iVAS")
            return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

        # 3. Test solve
        out.append("--- 3. TEST SOLVE TURNSTILE ---")
        r_ct = requests.post("https://api.capsolver.com/createTask", json={
            "clientKey": _CAPSOLVER_KEY,
            "task": {
        ...
        d_ct = r_ct.json()
        task_id = d_ct.get("taskId", "")
        out.append(f"createTask    : {d_ct}")

        if not task_id:
            out.append(f"⚠ GAGAL createTask: {d_ct.get('errorCode')} — {d_ct.get('errorDescription','')}")
            return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

        out.append(f"taskId        : {task_id}")
        out.append("Polling...")
        for i in range(20):
            time.sleep(3)
            r_gr = requests.post("https://api.capsolver.com/getTaskResult", json={
                "clientKey": _CAPSOLVER_KEY,
                "taskId":    task_id,
            }, timeout=15)
            d_gr = r_gr.json()
            status = d_gr.get("status", "")
            out.append(f"  poll #{i+1:02d}: status={status} err={d_gr.get('errorCode','')} {d_gr.get('errorDescription','')}")
            if status == "ready":
                token = d_gr.get("solution", {}).get("token", "")
                out.append(f"✅ SOLVED! token={token[:60]}...")
                break
            if status not in ("processing", "idle"):
                out.append(f"⚠ FATAL POLL: {d_gr}")
                break
        else:
            out.append("⚠ TIMEOUT setelah 60 detik polling")
    except Exception as e:
        import traceback
        out.append(f"ERROR: {e}")
        out.append(traceback.format_exc())
...
@app.route("/debug/cf-login")
def debug_cf_login():
    """
    Test login iVAS dengan 3 mode:
      ?mode=no_token   → POST tanpa cf-turnstile-response sama sekali (default)
      ?mode=empty      → POST dengan cf-turnstile-response=""
      ?mode=capsolver  → Pakai CapSolver (perlu balance)
    """
    acc_email = request.args.get("email", "").strip()
    mode      = request.args.get("mode", "no_token").strip()
    target    = next((a for a in ACCOUNTS if a["email"] == acc_email), ACCOUNTS[0])
    SEP = "=" * 60
    out = []
    out.append("[DEBUG CF-LOGIN]")
    out.append(f"Account : {target['email']}")
    out.append(f"Mode    : {mode}  (options: no_token | empty | capsolver)")
    out.append(SEP)
    try:
        sc = build_scraper()
        ...
        tok_el  = soup1.find("input", {"name": "_token"})
        tok     = tok_el["value"] if tok_el else None
        ts_div  = soup1.find("div", class_="cf-turnstile")
        sitekey = ts_div.get("data-sitekey", "") if ts_div else ""
        if not sitekey:
        ...
        out.append(f"GET status   : {r1.status_code}")
        out.append(f"_token       : {'FOUND: '+tok[:30]+'...' if tok else 'MISSING!'}")
        out.append(f"sitekey      : {'FOUND: '+sitekey[:40] if sitekey else 'NOT FOUND'}")
        out.append(f"Turnstile div: {'YES' if ts_div else 'NO'}")
        out.append("")

        if not tok:
            out.append("STOP: Tidak ada _token")
            return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

        # ── Siapkan login data sesuai mode ──
        login_data = {
            "email":    target["email"],
        ...
        if mode == "no_token":
            out.append("Mode: POST tanpa cf-turnstile-response (field tidak dikirim sama sekali)")

        elif mode == "empty":
            login_data["cf-turnstile-response"] = ""
            out.append("Mode: POST dengan cf-turnstile-response='' (string kosong)")

        elif mode == "capsolver":
            out.append("Mode: CapSolver — solving...")
            ts_token = _solve_turnstile(sitekey, LOGIN_URL) if sitekey else ""
            if ts_token:
                login_data["cf-turnstile-response"] = ts_token
                out.append(f"Turnstile solved : {ts_token[:50]}...")
            else:
                out.append("Turnstile FAILED — lanjut POST tanpa token")

        out.append(f"Fields dikirim: {list(login_data.keys())}")
        out.append("")
        ...
        login_ok = "/portal" in r2.url
        out.append(f"POST status  : {r2.status_code}")
        out.append(f"POST url     : {r2.url}")
        out.append(f"Login OK     : {'YES ✅' if login_ok else 'NO ❌'}")
        out.append("")

        # Cari pesan error dari HTML response
        soup2 = BeautifulSoup(h2, "html.parser")

        # Cari alert/error message dari Laravel
        for sel in [".alert", ".alert-danger", ".error", '[class*="error"]', '[class*="alert"]']:
            el = soup2.select(sel)
            if el:
                out.append(f"Error msg ({sel}): {el[0].get_text(strip=True)[:200]}")

        # Cari validation errors
        for li in soup2.select(".invalid-feedback, .help-block, ul.errors li"):
            out.append(f"Validation: {li.get_text(strip=True)[:200]}")

        # Cek apakah ada turnstile error di HTML
        if "turnstile" in h2.lower():
            out.append("⚠ Kata 'turnstile' ditemukan di response — Turnstile mungkin wajib!")
        if "captcha" in h2.lower():
            out.append("⚠ Kata 'captcha' ditemukan di response")

        out.append("")
        out.append(f"HTML snippet (500 char):")
        out.append(h2[:500])
        out.append("")
        ...
        if not login_ok:
            ...
            if mode == "no_token":
                out.append("  → Coba: ?mode=empty&email=" + target["email"])
                out.append("  → Atau: ?mode=capsolver&email=" + target["email"])
            elif mode == "empty":
                out.append("  → Turnstile sepertinya wajib. Perlu solver atau Playwright.")
            out.append("─" * 40)
...
@app.route("/debug/playwright-login")
def debug_playwright_login():
    """Test login Playwright headless langsung — bypass Turnstile gratis."""
    acc_email = request.args.get("email", "").strip()
    target    = next((a for a in ACCOUNTS if a["email"] == acc_email), ACCOUNTS[0])
    SEP = "=" * 60
    out = []
    out.append("[DEBUG PLAYWRIGHT LOGIN]")
    out.append(f"Account   : {target['email']}")
    out.append(f"Playwright: {'AVAILABLE ✅' if _PLAYWRIGHT_AVAILABLE else 'NOT AVAILABLE ❌'}")
    out.append(SEP)

    if not _PLAYWRIGHT_AVAILABLE:
        out.append("ERROR: Playwright tidak terinstall!")
        out.append("Pastikan requirements.txt include playwright==1.44.0")
        out.append("Dan Dockerfile include: RUN playwright install chromium")
        return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

    out.append("Memulai Playwright headless browser...")
    out.append("(ini butuh 15-30 detik)")
    out.append("")

    start_time = time.time()
    try:
        result = _solve_turnstile_playwright(LOGIN_URL, target["email"], target["password"])
        elapsed = round(time.time() - start_time, 1)

        out.append(f"Selesai dalam {elapsed} detik")
        out.append("")

        if result["ok"]:
            out.append("LOGIN OK ✅")
            out.append(f"CSRF token  : {result.get('csrf','')[:40]}...")
            out.append(f"recv_csrf   : {result.get('recv_csrf','')[:40]}...")
            out.append(f"Cookies     : {len(result.get('cookies',[]))} cookie(s)")
            out.append(f"live_html   : {len(result.get('live_html',''))} chars")
            out.append("")
            out.append("✅ Playwright login berhasil! Login production akan otomatis pakai ini.")
        else:
            out.append(f"LOGIN GAGAL ❌")
            out.append(f"Error: {result.get('error','unknown')}")
            out.append("")
            out.append("Kemungkinan penyebab:")
            out.append("  - Email/password salah")
            out.append("  - Chromium tidak bisa akses ivasms.com dari Railway")
            out.append("  - Timeout (Cloudflare block headless browser)")
