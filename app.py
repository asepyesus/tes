...
# ── CapSolver key untuk bypass Cloudflare Turnstile ──
_CAPSOLVER_KEY = os.getenv("CAPSOLVER_KEY", "CAP-397E0F6106840264406AC238022E8A3C4B5DF76249ABCD3176B5A4DE39DC457E").strip()
...
def _solve_turnstile(sitekey, page_url):
    """Legacy CapSolver — masih dipakai di debug endpoint. Login utama pakai Playwright."""
    if not _CAPSOLVER_KEY:
        return ""
    try:
        logger.info(f"[TURNSTILE] Solving via CapSolver... sitekey={sitekey[:30]}")
        r1 = requests.post("https://api.capsolver.com/createTask", json={
        ...
        d1 = r1.json()
        task_id = d1.get("taskId", "")
        if not task_id:
            logger.error(f"[TURNSTILE] createTask gagal: {d1}")
            return ""
        for _ in range(30):
            time.sleep(2)
            ...
            if d2.get("status") == "processing":
                continue
            logger.error(f"[TURNSTILE] Error: {d2.get('errorCode')} {d2.get('errorDescription')}")
            return ""
...
def login_account(account):
    """
    Login satu akun via Playwright headless (bypass Turnstile gratis).
    Fallback ke requests+CapSolver kalau Playwright tidak tersedia.
    Return dict: {ok, scraper, csrf, recv_csrf, live_html, email}
    """
    email    = account["email"]
    password = account["password"]

    # ── UTAMA: Playwright headless ──
    if _PLAYWRIGHT_AVAILABLE:
        logger.info(f"[LOGIN] Playwright mode untuk {email}")
        result = _solve_turnstile_playwright(LOGIN_URL, email, password)
        if result["ok"]:
            # Build requests session dengan cookies dari Playwright
            ...
            return {
                ...
                "scraper":   scraper,
                "csrf":      result["csrf"],
                "recv_csrf": result["recv_csrf"],
                "live_html": result["live_html"],
                "email":     email,
            }
        else:
            logger.warning(f"[LOGIN] Playwright gagal: {result['error']} — fallback ke CapSolver")

    # ── FALLBACK: requests + CapSolver ──
    logger.info(f"[LOGIN] CapSolver fallback untuk {email}")
    scraper = build_scraper()
    try:
        ...
        if "/login" in getattr(resp, "url", ""):
            return {"ok": False, "error": "Login gagal (Turnstile required, CapSolver gagal)", "email": email}
...
def debug_capsolver():
    ...
    SEP = "=" * 60
    out = []
    out.append("[DEBUG CAPSOLVER]")
    out.append(f"Key (partial) : {_CAPSOLVER_KEY[:25]}...")
    out.append(SEP)
    try:
        # 1. Cek balance
        out.append("--- 1. GET BALANCE ---")
        rb = requests.post("https://api.capsolver.com/getBalance", json={
            "clientKey": _CAPSOLVER_KEY
        }, timeout=10)
        db = rb.json()
